...
def quick_search(spark, parquet_path):
    ...
    try:
        ...
        if data:
            ...
            print(f"\nNo anime found matching '{raw_query}'. Try a different word or check spelling.")
...
def get_rankings(spark, parquet_path):
    ...
    year_input = input("Enter Year (e.g., 2024, or press Enter for all years): ").strip()
    genre_input = input("Enter Genre (e.g., Action, or press Enter for all genres): ").strip().capitalize()
    ...
    results = filtered_df.orderBy(F.desc("score")).limit(10)
    data = results.collect()
...
def industry_trends(spark, parquet_path):
    ...
    try:
        ...
        return
    # Processing: Explode studios, aggregate, and filte explode because one anime can have multiple studios
...
def surprise_me(spark, parquet_path):
    ...
    try:
        df = spark.read.parquet(parquet_path)
        # Filter for High Quality (Score > 8.0)
        ...
        if random_pick:
            ...
            print(f"RATING        | {row['rating'] or 'Not Rated'}")
            print("-" * 30)
            print("If you don't like this one, run 'Surprise Me' again!")
        else:
...
def display_data_dictionary():
    ...
    dictionary = [
        ...
        ("favorites", "Integer", "Total users who favorite the entry"),
