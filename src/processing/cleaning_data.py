...
def clean_anilist(input_df):
    ...
    normalized_target_list = [s.lower().strip() for s in studios_list]
     ...
     # Initial selection and numeric standardization
    ...
    # Intersect the normalized data with normalized list
    ...
    F.when((F.size(F.col("matched_studios")) > 0),
    F.col("matched_studios")).otherwise(F.array(F.lit("Unknown"))))
...
def clean_kaggle(input_df):
    working_df = input_df.select(
        ...
        F.when(F.col("Score") == "UNKNOWN", 0.0).otherwise(F.col("Score")).cast("double").alias("score"),
        F.col("Genres").alias("raw_genres"),
        F.col("Type").cast("string").alias("type"),
        F.when(F.col("Episodes") == "UNKNOWN", 0.0).otherwise(F.col("Episodes")).cast("double").alias("episodes"),
    ...
    # Create temporary columns that extract the first 4-digit number found
    ...
    # Use Coalesce to prioritize Premiered, then Aired, then default to 0 Coalesce picks the first non-null value it sees
...
def integrate_datasets(j_df, a_df, k_df):
    ...
    # Alias columns to keep track of sources j, a, k
    ...
    # Perform Full Outer Joins on mal_id and use coalesce in the join condition to handle cases where Jikan might be missing an ID
    combined = j.join(a, j.j_mal_id == a.a_mal_id, "full_outer").join(k, F.coalesce(F.col("j_mal_id"), F.col("a_mal_id")) == k.k_mal_id, "full_outer")

    # Build the Master Record using coalesce Priority order: Jikan (j), then AniList (a), then Kaggle (k)
    final_df = combined.select(
        ...
        # The average of all non-null scores
        F.expr("""
        ...
        F.coalesce(F.col("j_themes"), F.col("k_themes")).alias("themes"),
        F.coalesce(F.col("j_rank"), F.col("k_rank")).alias("rank"),
        F.coalesce(F.col("j_rating"), F.col("k_rating")).alias("rating")
    )
    # Clean up math edge cases and force double types
